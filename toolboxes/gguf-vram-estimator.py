...
def run_estimator(gguf_file: str, context_sizes: List[int], overhead_gib: float):
    try:
        ...
        print(
            f"Incl. Overhead: {overhead_gib:.2f} GiB (for compute buffer, etc. adjustable via --overhead)"
        )
        ...
        print(
            f"{'Context Size':>15s} | {'Context Memory':>15s} | {'Est. Total VRAM':>15s}"
        )
        print("-" * 51)
        for n_ctx in context_sizes:
            mem_full = n_ctx * n_layers_full * bytes_per_token_per_layer
            mem_swa = (
                min(n_ctx, swa_window_size) * n_layers_swa * bytes_per_token_per_layer
            )
            kv_cache_bytes = mem_full + mem_swa
            total_bytes = model_size_bytes + kv_cache_bytes + overhead_bytes
            print(
                f"{n_ctx:>15,} | {format_mem(kv_cache_bytes):>15s} | {format_mem(total_bytes):>15s}"
            )
...
def main():
    ...
    parser.add_argument(
        "gguf_file",
        help="Path to the GGUF model file (any part of a multi-part model).",
    )
    parser.add_argument(
        "-c",
        "--contexts",
